...
def train(info):
    train_set, test_set = load_datasets(info['dataset_path'], info['dataset_name'])
    train_loader, test_loader = make_dataloaders_with_batchSize(train_set, test_set, info['batchSize'])
    ...
    for epoch in range(info['metaEpoch']):
        # train some epochs
        model.train()
        if c.verbose:
            print(F'\nTrain epoch {epoch}')
        for sub_epoch in range(info['subEpoch']):
    ...
    if c.save_model:
        model.to('cpu')
        save_model(model, info['model_name'])
